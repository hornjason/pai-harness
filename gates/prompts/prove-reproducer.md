...
6. Compare the before-state (provided) against the current after-state.
   ...
   the fix working.
6b. For S-tier evidence (bug fixes): apply negative control — revert the
   fix temporarily, confirm the bug returns, then re-apply. This proves
   the fix is the actual cause of the improvement, not a coincidence.
