...
describe("AC-6: grade output includes timing", () => {
  ...
  test("compliance-grade.json has timing array with role and durationSeconds", async () => {
    const proc = Bun.spawnSync(
      ["bun", GRADE_SCRIPT, "--transcripts", FIXTURES, tmpDir],
      { stdout: "pipe", stderr: "pipe" }
    );
  ...
  test("timing entry roles match graded agent roles", async () => {
    const proc = Bun.spawnSync(
      ["bun", GRADE_SCRIPT, "--transcripts", FIXTURES, tmpDir],
      { stdout: "pipe", stderr: "pipe" }
    );
    ...
    const gradedRoles = gradeOutput.grades.map((g: any) => g.role);
    const timingRoles = gradeOutput.timing.map((t: any) => t.role);

    expect(timingRoles).toEqual(gradedRoles);
  });
});
